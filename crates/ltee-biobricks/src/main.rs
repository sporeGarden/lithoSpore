...
fn compute_growth_rate(measurements: &[GrowthMeasurement]) -> Option<f64> {
    ...
    // Find log-phase window (OD 0.02–0.3) for exponential fit
...
fn run_tier2_rust(cli: &Cli, start: Instant) -> ModuleResult {
    ...
    // T08: Real burden computation from plate data
    if has_plate_data {
        ...
        // Check 6: plate data is parseable
        total += 1;
        ...
        if growth_rates.len() >= 5 {
            // Check 7: growth rates are positive
            total += 1;
            ...
            // Check 8: compute burden distribution statistics
            ...
            // T08: Fat-tailed check — kurtosis > 3 (excess kurtosis > 0)
            if burdens.len() >= 10 {
                let m4 = burdens.iter()
