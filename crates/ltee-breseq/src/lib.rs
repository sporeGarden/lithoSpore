...
fn run_tier2_rust(_data_dir: &str, expected_path: &str, start: Instant) -> ModuleResult {
    ...
    eprintln!(
        "  [{}] 12 replicate populations: {n_pop}",
    ...
    eprintln!(
        "  [{}] 264 sequenced genomes: {n_genomes}",
    ...
    eprintln!(
        "  [{}] Genome length: {genome_len:.0} bp (expected {expected_len:.0} ± {genome_tol:.0})",
        if len_ok { "PASS" } else { "FAIL" }
    );

    total += 1;
    let rate = targets["nonmutator_rate_per_bp_per_gen"]["value"]
        .as_f64()
        .unwrap_or(0.0);
    let rate_tol = targets["nonmutator_rate_per_bp_per_gen"]["tolerance"]
        .as_f64()
        .unwrap_or(1e-11);
    ...
    eprintln!(
        "  [{}] Non-mutator rate: {rate:.2e} per bp/gen (expected 8.9e-11 ± {rate_tol:.0e})",
        if rate_ok { "PASS" } else { "FAIL" }
    );

    total += 1;
    let muts_50k = targets["nonmutator_mutations_at_50k"]["value"]
        .as_f64()
        .unwrap_or(0.0);
    let muts_tol = targets["nonmutator_mutations_at_50k"]["tolerance"]
        .as_f64()
        .unwrap_or(2.3);
    ...
    eprintln!(
        "  [{}] Mutations at 50k: {muts_50k:.1} (expected 20.6 ± {muts_tol:.1})",
    ...
    eprintln!(
        "  [{}] Ts/Tv ratio: {ts_tv:.2} (expected 1.7 ± {ts_tv_tol:.1})",
        if ts_tv_ok { "PASS" } else { "FAIL" }
    );

    total += 1;
    let gc_at = targets["gc_to_at_fraction"]["value"]
        .as_f64()
        .unwrap_or(0.0);
    let gc_at_tol = targets["gc_to_at_fraction"]["tolerance"]
        .as_f64()
        .unwrap_or(0.05);
    ...
    eprintln!(
        "  [{}] GC→AT fraction: {gc_at:.2} (expected 0.68 ± {gc_at_tol:.2})",
        if gc_at_ok { "PASS" } else { "FAIL" }
    );

    if let Some(spectrum_val) = targets.get("mutation_spectrum")
        && let Some(spectrum) = spectrum_val.get("value").and_then(|v| v.as_object())
    {
        ...
        for (class, expected_frac) in &expected_spectrum {
            if let Some(observed) = spectrum.get(*class).and_then(serde_json::Value::as_f64) {
                ...
                eprintln!(
                    "  [{}] Spectrum {class}: {observed:.3} (expected {expected_frac:.3} ± {spec_tol:.3})",
                    if class_ok { "PASS" } else { "FAIL" }
                );
            }
        }

        let total_frac: f64 = spectrum
            .values()
        ...
        eprintln!(
            "  [{}] Spectrum sums to ~1.0: {total_frac:.4}",
    ...
    let gens: Vec<f64> = curve["generations"]
        .as_array()
        .map(|a| {
            a.iter()
        ...
        .unwrap_or_default();
    let muts: Vec<f64> = curve["expected_mutations_nonmutator"]
        .as_array()
        .map(|a| {
            a.iter()
                .filter_map(serde_json::value::Value::as_f64)
    ...
    let linear_ok = if gens.len() >= 3 && gens.len() == muts.len() {
        let pos_gens: Vec<f64> = gens
            .iter()
            .zip(&muts)
            .filter(|&(&g, _)| g > 0.0)
            .map(|(&g, _)| g)
            .collect();
        let pos_muts: Vec<f64> = gens
            .iter()
            .zip(&muts)
            .filter(|&(&g, _)| g > 0.0)
            .map(|(_, &m)| m)
        ...
        eprintln!(
            "  [{}] Near-linear accumulation (Pearson r={r:.6}, min 0.99)",
    ...
    if gens.len() >= 2 && muts.len() >= 2 {
        ...
        if last_gen > 0.0 && genome_len > 0.0 {
            ...
            eprintln!(
                "  [{}] Computed rate matches published: {computed_rate:.2e} vs {rate:.2e} (< 10% deviation)",
