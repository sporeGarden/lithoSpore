...
pub fn run_validation(data_dir: &str, expected: &str, max_tier: u8) -> ModuleResult {
    ...
    if max_tier < 2 {
        return harness::tier0_structural("citrate_innovation", expected, start);
    }
        ...
        .and_then(serde_json::Value::as_f64)
    {
        ...
        eprintln!(
            "  [{}] Cit+ fraction: {frac:.4} (expected {expected_frac:.4} ± 0.01)",
            if frac_ok { "PASS" } else { "FAIL" }
        );
    }

    if let Some(pot) = source
        ...
        .and_then(serde_json::Value::as_f64)
    {
        ...
        eprintln!(
            "  [{}] Potentiation fraction: {pot:.4} (in (0, 1])",
    ...
    if let Some(pg) = pot_gen {
        ...
        eprintln!(
            "  [{}] Mean potentiation generation: {pg:.0} (expected 30000–50000)",
    ...
    if let Some(cg) = cit_gen {
        ...
        eprintln!(
            "  [{}] Mean Cit+ generation: {cg:.0} (expected 40000–55000)",
    ...
    if let (Some(pg), Some(cg)) = (pot_gen, cit_gen) {
        ...
        eprintln!(
            "  [{}] Potentiation window: {window:.0} generations (Cit+ gen - potentiation gen, expected < 10000)",
        ...
        eprintln!(
            "  [{}] Potentiation precedes innovation: {pg:.0} < {cg:.0}",
            if order_ok { "PASS" } else { "FAIL" }
        );
    }

    if let Some(replay) = source
        .get("replay_probabilities")
        .and_then(|v| v.as_object())
    {
        checks += 1;
        let all_valid = replay
            .values()
            .all(|v| v.as_f64().is_some_and(|p| (0.0..=1.0).contains(&p)));
        if all_valid {
            passed += 1;
        }
        eprintln!(
            "  [{}] Replay probabilities all in [0, 1]: {all_valid}",
            if all_valid { "PASS" } else { "FAIL" }
        );

        checks += 1;
        let early_zero = replay
            .iter()
        ...
        eprintln!(
            "  [{}] Early replays (< 20k gen) have zero probability: {early_zero}",
            if early_zero { "PASS" } else { "FAIL" }
        );
    }

    if let Some(two_hit) = source.get("two_hit_model")
        && let Some(window_gens) = two_hit
            .get("potentiation_window_gens")
            .and_then(serde_json::Value::as_f64)
    {
        checks += 1;
        ...
        eprintln!(
            "  [{}] Two-hit model window: {window_gens:.0} gens (expected 1000–10000)",
            if window_ok { "PASS" } else { "FAIL" }
        );
    }

    let single = source
        .get("single_hit_mean_wait")
        .and_then(serde_json::Value::as_f64);
    let two_hit_val = source
        .get("two_hit_analytical_mean")
        .and_then(serde_json::Value::as_f64);
    if let (Some(s), Some(t)) = (single, two_hit_val) {
        ...
        eprintln!(
            "  [{}] Two-hit wait >> single-hit: {t:.0} > 10×{s:.0}",
            if order_ok { "PASS" } else { "FAIL" }
        );
    }

    let empirical = source
        .get("two_hit_empirical_mean")
        .and_then(serde_json::Value::as_f64);
    if let (Some(e), Some(a)) = (empirical, two_hit_val) {
        ...
        eprintln!(
            "  [{}] Empirical < analytical two-hit: {e:.0} < {a:.0}",
...
mod tests {
    ...
    fn test_tier0_structural() {
        let dir = std::env::temp_dir().join("ltee_citrate_tier0_test");
        let _ = std::fs::create_dir_all(&dir);
        let expected = dir.join("expected.json");
        std::fs::write(&expected, r#"{"validation_checks":[{"name":"cit_plus"}]}"#).unwrap();
