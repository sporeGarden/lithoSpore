...
pub fn run(
    ...
    // 6. Generate tolerances.toml (profile-driven when available)
    ...
    // 8. Generate automation scripts (profile-driven when available)
    ...
    // Final: generate README (profile-driven when available)
...
fn generate_tolerances(scope: &toml::Table, _profile: Option<&DomainProfile>) -> String {
    let mut output = String::from("# lithoSpore acceptance tolerances\n# Generated by `litho promote`\n\n");
    ...
    if let Some(modules) = scope.get("module").and_then(|v| v.as_array()) {
        for module in modules {
            if let Some(name) = module.get("name").and_then(|v| v.as_str()) {
                ...
                output.push_str("tier1_rmsd_kj_max = 2.0\n");
                output.push_str("tier2_rmsd_kj_max = 2.0\n\n");
            }
        }
    } else {
...
fn write_scripts(root: &Path, profile: Option<&DomainProfile>) {
...
"#;
    ...
    let translate_sh = if profile.map(|p| p.translation.enabled).unwrap_or(true) {
        r#"#!/bin/bash
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
...
fn generate_reproduce_script(profile: Option<&DomainProfile>) -> String {
    ...
    if let Some(p) = profile {
        if p.derivation.contracts.is_empty() {
            script.push_str("echo \"  No derivation contracts defined in profile.\"\n");
            script.push_str("exit 0\n");
            return script;
        }

        let tool = &p.derivation.tool;
        script.push_str(&format!("echo \"  Requires: {tool}\"\n"));
        ...
        script.push_str("rm -rf \"$TMPDIR\"\necho \"=== Reproduce complete: $PASS passed, $FAIL failed ===\"\n");
    } else {
        script.push_str("echo \"  No domain profile found — cannot auto-generate derivation commands.\"\n");
...
fn generate_chassis_readme(name: &str, version: &str, ps_version: &str, profile: Option<&DomainProfile>) -> String {
    let tools_str = profile
        .map(|p| p.tools.join(", "))
        .unwrap_or_else(|| "domain-specific tools".to_string());

    let reproduce_requires = profile
        .map(|p| p.derivation.tool.clone())
        .filter(|t| !t.is_empty())
        .unwrap_or_else(|| "domain-specific tool".to_string());

    let has_translation = profile.map(|p| p.translation.enabled).unwrap_or(true);
